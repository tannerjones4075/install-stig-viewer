...
# CONFIGURATION
STIG_VERSION="3-5-1"
STIG_ZIP="U_STIGViewer-linux_x64-${STIG_VERSION}.zip"
...
verify_checksum() {
  ...
  expected_sha256=$(grep -A 1 "U_STIGViewer-linux_x64-${STIG_VERSION}.zip" "$tmp_hash_file" | \
                    grep "SHA256:" | \
                    awk '{print $2}' | \
                    tr -cd '[:alnum:]' | \
                    tr '[:lower:]' '[:upper:]')
